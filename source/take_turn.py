...
def take_turn(player, players, game_board):
    ...
    can_recall = True if player.yellow_building else False
    if can_recall and len(player.assist_locs) > 0 and sanitised_inputs.input_yes_no("Do you want to use your yellow mosque action?"):
        ...
        if player.remove_lira(2) and game_board.tile[assist_loc].remove_assistant(player.color): # TODO: Fix if one is True and the other is False
            player.assistants += 1
            can_recall == False
